  ...
  MultiSelect.prototype = {
    ...
    'select_all' : function(){
        var ms = this.$element,
            obj = {},
            values = ms.val();
        if(this.$selectableUl.find('.ms-elem-selectable').length === 0){
          return;
        }

        ms.find('option:not(":disabled")').prop('selected', true);
        var $selectable_elm = this.$selectableUl.find('.ms-elem-selectable').not("." + this.options.disabledClass);
        var $selection_elm = this.$selectionUl.find('.ms-elem-selection').not('.' + this.options.disabledClass)
        $selectable_elm.hide().addClass('ms-selected');
        $selection_elm.show().addClass('ms-selected');


        /*this.$selectableUl.find('.ms-elem-selectable').filter(':not(.' + this.options.disabledClass + ')').addClass('ms-selected').each(function(index){
          if($(this).is(":visible")){
            obj[index] = true
          }
          $(this).hide();
        ...
        this.$selectionUl.find('.ms-elem-selection').filter(':not(.' + this.options.disabledClass + ')').addClass('ms-selected').each(function(_index,_val){
          var that = this;
          if(obj[_index]){
            $(that).show()
          }
    ...
    'deselect_all' : function(){
        ...
        this.$selectableUl.find('.ms-elem-selectable').show();
        this.$selectionUl.find('.ms-elem-selection').hide();
