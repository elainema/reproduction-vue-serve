...
export default {
  data(){
    ...
    methods: {
        ...
        getArticleDetail(){
            Vue.http.get('Article/getArticleDetail', {params:{id: this.$route.query.id, source:"not_view"}}).then(response => {
                var data = response.body && response.body.data 
        ...
        editArticle() {
            ...
            this.$http.post('Article/editArticle', options).then(response => {
                this.loadingZone = false
                that.showAlert("success!","success")
                setTimeout(function(){
                    that.$router.push("./update_help_center")
                },2000)
