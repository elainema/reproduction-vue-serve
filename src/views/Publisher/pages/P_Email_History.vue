<template>
  <div :class="loadingZone ? 'ajax_loading' : '' ">
                    ...
                    <form class="form-inline" ref="form" @submit.prevent="onSubmit">
                        <div class="row-fluid">
                            <div class="col-md-6 form-inline input-daterange input-group" id="datepicker">
                                <dateTimePicker_range  :from_date="'lmonth_minus'"  :startDate="startDate" :endDate="endDate" :show_date_shortcut=false :no_hour=true class="col-md-12" :start_class="'start_time'" :end_class="'end_time'" :limitRange="'one_month'"></dateTimePicker_range>
                ...
                <table class="table table-hover tablesorter list-table" id="email_table">
                    ...
                    <tbody>
                        <tr v-for="item in email_list" v-if="email_list.length">
                            <td>{{item.id}}</td>
                            <td><a href="javascript:;"  @click="showMailTo(item)" class="underline">{{item.to}} Recipients</a></td>
    ...
    <!-- <mailContentModal :modalState="modalState" :hideModal="hideModal" :sysTemEmail="sysTemEmail"></mailContentModal> --><mail-content-modal
    ...
    methods: {
        ...
        onSubmit(){
            ...
            this.$nextTick(function(){
                ...
                if(!start_time.val().match(/00:00:00/g)) {
                    start_time.val(start_time.val() + " 00:00:00")
                    end_time.val(end_time.val() + " 23:59:59")
                }


                Vue.http.get('Email/getSystemEmailList',{params:$(this.$refs.form).getFormQuery()}).then(response => {
        ...
        getSystemEmailToById(item, callback){
            let params = {
                email_to_id: item.id,
                status:"all",
                start_time: moment().add(-1, "month").format("YYYY-MM-DD") + " 00:00:00",
                end_time:moment().format("YYYY-MM-DD") + " 23:59:59"

            }
            Vue.http.get('Email/getSystemEmailToById',{params:params}).then(response => {
                let data  = response.body.data 
        ...
        showMailTo(item) {
            let that = this
            this.getSystemEmailToById(item, function(data){
