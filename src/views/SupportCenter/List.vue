                ...
                <div class="row well" id="searchForm">
                    <div class="col-md-12" role="main">
                        <form class="form-horizontal frm-entity form-inline form-help-center" ref="form" @submit.prevent="getAllArticleList">
                            ...
                            <div class="form-group-block mt15">
                                <div class="col-md-6">
                                    <router-link to="/edit_support_center" class="col-md-3 btn btn-primary ">Add Page</router-link>
                ...
                <table id="pageTable" class="mt40  table table-bordered table-hover">
                    ...
                    <tbody>
                        <tr v-for="item in articleList">
                            ...
                            <td>{{item.hit}}</td>
                            <td>{{item.status}}</td>
                            <td><router-link :to="'/edit_support_center?id=' + item.id + '&source=not_view'">View & Edit</router-link></td>
                        </tr>
                    </tbody>
                </table>
                </div>
...
export default {
  data(){
    ...
    components:{AlertPop, catalogModal},
    computed:{
    },
    watch:{
    ...
    methods: {
        getAllArticleList() {
            ...
            Vue.http.get('ArticlePublisher/getAllArticleList', {params: $(this.$refs.form).getFormQuery()}).then(response => {
        ...
        onAdd(name) {
            Vue.http.post('ArticlePublisher/editCategory', {act:"add", name:name}).then(response => {
                var data = response.body && response.body.data 
        ...
        onRemove(id) {
            Vue.http.post('ArticlePublisher/editCategory', {act:"delete", id:id}).then(response => {
                var data = response.body && response.body.data 
        ...
        onEdit(id, name){
            ...
            Vue.http.post('ArticlePublisher/editCategory', {act:"edit", id:id, name: name}).then(response => {
                var data = response.body && response.body.data 
